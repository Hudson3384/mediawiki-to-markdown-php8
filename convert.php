...
if (!empty($arguments['output'])) {
  ...
  if (!file_exists($output_path)) {
    ...
    mkdir($output_path, 0777, true); // Set the recursive parameter to true
  }
}
...
foreach ($result as $node) {
  ...
  if (!empty($directory)) {
    if (!file_exists($directory)) {
      mkdir($directory, 0777, true); // Set the recursive parameter to true
    }
    $directory .= '/'; // Ensure the directory path ends with a slash
  }
  ...
  if ($file) {
    ...
    echo "Failed to open file: " . normalizePath($directory . $filename . '.md') . PHP_EOL;
  }
